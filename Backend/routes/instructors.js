...
router.post('/add', function (req, res) {
    ...
    mongoClient.connect(url, { useUnifiedTopology: true }, (err, db) => {
        if (err) throw err;
        let dbo = db.db('gym');
        dbo.collection('instructors').insertOne(form, (err, result) => {
            if (err) throw err;
            db.close();
            res.end();
        });
...
router.get('/list', function (req, res) {
    mongoClient.connect(url, { useUnifiedTopology: true }, (err, db) => {
        if (err) throw err;
        let dbo = db.db('gym');
        dbo.collection('instructors').find().toArray((err, result) => {
            if (err) throw err;
            db.close();
...
router.put('/update/:updateid', function (req, res) {
    ...
    mongoClient.connect(url, { useUnifiedTopology: true }, (err, db) => {
        ...
        dbo.collection('instructors').updateOne(query, newValues, (err, result) => {
            if (err) throw err;
            db.close();
            res.end();
        });
...
router.delete('/delete/:id', function (req, res) {
    mongoClient.connect(url, {
        ...
        dbo.collection('instructors').deleteOne(param, (err, result) => {
            if (err) throw err;
            db.close();
            res.end();
        });
