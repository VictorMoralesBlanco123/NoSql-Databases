...
router.post('/add', function (req, res) {
    ...
    mongoClient.connect(url, { useUnifiedTopology: true }, (err, db) => {
        if (err) throw err;
        let dbo = db.db('gym');
        dbo.collection('groups').insertOne(form, (err, result) => {
            if (err) throw err;
            db.close();
            res.end();
        });
...
router.get('/search/:param', (req, res) => {
    ...
    mongoClient.connect(url, { useUnifiedTopology: true }, (err, db) => {
        ...
        dbo.collection('groups').find(query).toArray((err, result) => {
            if (err) throw err;
            db.close();
            res.send(JSON.stringify(result));
            res.end();
...
router.put('/update/:updateid', function (req, res) {
    ...
    mongoClient.connect(url, { useUnifiedTopology: true }, (err, db) => {
        ...
        dbo.collection('groups').updateOne(query, newValues, (err, result) => {
            if (err) throw err;
            db.close();
            res.end();
        });
...
router.delete('/delete/:id', function (req, res) {
    mongoClient.connect(url, {
        ...
        dbo.collection('groups').deleteOne(param, (err, result) => {
            if (err) throw err;
            db.close();
            res.end();
        });
